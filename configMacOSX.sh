...
brew install jq bash tree tmux wget lz4 wget curl rsync octave gnuplot ansible openvpn speedtest_cli 

# Install gnu dev tools
brew install make autoconf gawk gcc gnu-sed coreutils colordiff 

# Installs more goodies
brew install go graphviz poppler consul
pip install Flask
